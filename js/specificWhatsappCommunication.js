function WhatsappDatabase(input){
				    ...
				    .post(
				        JSON.stringify({
						  "statements" : [ {
							...
							FetchWhatsappCommunicationDetail(resultArr,input);
						});
}

function FetchWhatsappCommunicationDetail(resultArr,input){
	var nodeArr = resultArr[0];
	var linkArr = resultArr[1];
	var groupArr = resultArr[2];
	var inputPhoneNumber = $("#phoneNo").val();

	var _query = "MATCH (n:WHATSAPP)-[r:Whatsappchat]->(m:WHATSAPP) WHERE n.PhoneNumber = '" + inputPhoneNumber + "' OR m.PhoneNumber = '" + inputPhoneNumber + "' RETURN distinct r ORDER BY r.Date,r.Time ASC";
	console.log(_query);
